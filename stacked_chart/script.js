...
function sortStudentsByTotalTimeSpent() {
  return allStudentNames.sort((a, b) => {
...
const sortedStudents = sortStudentsByTotalTimeSpent();
...
function createDatasetFor(studentNames) {
  return modules.map(({ id, label, backgroundColor }) => {
    return {
      label,
      backgroundColor,
      data: [...getStudentsTimeSpentFor(studentNames, id)],
    };
  });
}
...
const data = {
  labels: labels,
  datasets: createDatasetFor(sortedStudents),
};
...
$(function () {
  $('#ms').multipleSelect({
    onClose: function () {
      ...
      updateChart(selectedStudentNames);
    },
    width: '100%',
    filter: true,
  });
...
function populateStudentDropdown() {
  ...
  $.each(sortedStudents, function () {
    studentDropdown.append($('<option />').val(Math.random).text(this));
  });
}
